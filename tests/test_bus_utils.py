...
def test_create_enhanced_bus_locations_map(sample_locations_df):
    ...
    assert result.location == [sample_locations_df['lat'].mean(), sample_locations_df['lon'].mean()]

@patch('stride.get')
def test_get_routes_for_route_mkt(mock_stride_get):
    """Test the get_routes_for_route_mkt function."""
    mock_data = [
        {'route_mkt': '23056', 'route_long_name': 'רדינג Test', 'route_direction': '1', 'line_ref': 'L1'},
        {'route_mkt': '23056', 'route_long_name': 'Other Route', 'route_direction': '2', 'line_ref': 'L2'}
    ]
...
def test_get_current_distances_to_ref(sample_locations_df):
    ...
    for ride_id, info in result.items():
        ...
        assert all(key in info for key in ['current_distance', 'last_update', 'vehicle_ref', 'lat', 'lon'])
